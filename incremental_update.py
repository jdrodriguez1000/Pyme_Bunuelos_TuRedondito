...
TABLAS_DIARIAS = {
    'ventas': 'usr_ventas',
    'inventario': 'usr_inventario_detallado',
    'finanzas': 'usr_finanzas_pyme',
    'clima': 'usr_clima_diario',
    'marketing': 'usr_marketing_digital',
    'trm': 'usr_trm_diaria'
}

TABLAS_MENSUALES = {
    'usr_ipc_mensual': 'inflacion_mensual_ipc',
    'usr_desempleo_mensual': 'tasa_desempleo'
}
...
def get_last_state():
    inv = supabase.table('usr_inventario_detallado').select("*").order("fecha", desc=True).limit(1).execute()
    fin = supabase.table('usr_finanzas_pyme').select("*").order("fecha", desc=True).limit(1).execute()
    trm = supabase.table('usr_trm_diaria').select("*").order("fecha", desc=True).limit(1).execute()
...
def generate_daily_batch(start_date, end_date, state):
    ...
    for current_date in date_range:
        ...
        # E. INVENTARIO
        # Nueva lógica de recepción: Día 14 y último día hábil del mes
        es_14 = (current_date.day == 14)
        
        # Detectar si hoy es el último día hábil (Lunes a Viernes)
        es_ultimo_habil = False
        if wd < 5: # Solo si es día de semana
            # Buscamos el siguiente día laborable (saltando fines de semana)
            next_biz_day = current_date + timedelta(days=1)
            while next_biz_day.weekday() >= 5:
                next_biz_day += timedelta(days=1)
            # Si el siguiente día hábil ya pertenece al próximo mes, hoy es el último
            if next_biz_day.month != m:
                es_ultimo_habil = True

        recibido = 6000 if (es_14 or es_ultimo_habil) else 0
        
        # El consumo de kits es 1:1 con los buñuelos preparados (según documento histórico)
        preparados = demanda
        # Verificación de stock (no podemos preparar más de lo disponible)
        if preparados > (kit_ayer + recibido):
            preparados = kit_ayer + recibido
            
        kit_fin = kit_ayer + recibido - preparados
        ...
        batch['inventario'].append({
            ...
            'lbs_recibidas': int(recibido * 0.02),
            ...
            'unidades_agotadas': int(max(0, demanda - preparados)), 
...
def run_update():
    ...
    for table, col in TABLAS_MENSUALES.items():
        ...
        if last_m and last_m < target_m:
            ...
            while curr <= target_m:
                val = float(round(np.random.uniform(0.3, 0.9), 4) if table == 'usr_ipc_mensual' else round(np.random.uniform(9.0, 12.5), 2))
